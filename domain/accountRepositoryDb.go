...
func (c AccountRepositoryDb) Save(acc Account) (*Account, *errs.AppError) {
	sqlInsert := "INSERT INTO accounts (customer_id, opening_date, account_type, amount, status) values (?,?,?,?,?)"
	result, err := c.client.Exec(sqlInsert, acc.CustomerId, acc.OpeningDate, acc.AccountType, acc.Amount, acc.Status)
